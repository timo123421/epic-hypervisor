...
export default function HostTerminal() {
  ...
  const connect = () => {
    ...
    term.clear();
    term.writeln('\x1b[36m*** Connecting to Host Terminal... ***\x1b[0m');
    ...
    const wsUrl = `${protocol}//${window.location.host}/api/terminal?token=${token}`;
    ...
    ws.onclose = () => {
      ...
      term.writeln('\r\n\x1b[31m*** Disconnected from Host Terminal ***\x1b[0m');
    };
  ...
  return (
    <div className={`flex flex-col ${isFullscreen ? 'fixed inset-0 z-50 bg-slate-900 p-4' : 'h-[600px] card overflow-hidden'}`}>
      {/* Terminal Header */}
      <div className="bg-slate-950 px-4 py-3 flex items-center justify-between border-b border-slate-800 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-sm font-medium text-slate-200">Host Bash Terminal</h3>
